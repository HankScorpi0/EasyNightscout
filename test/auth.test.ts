import { describe, expect, it } from "vitest";
import { optionsResponse, requireReadAuth, requireWriteAuth } from "../src/auth";
import type { Env } from "../src/types";

const env = {
  API_SECRET: "secret",
  READ_PUBLIC: "false"
} as Env;

describe("auth", () => {
  it("accepts api-secret header", () => {
    const request = new Request("https://example.com/api/v1/entries", {
      method: "POST",
      headers: { "api-secret": "secret" }
    });
    expect(requireWriteAuth(request, env)).toBeNull();
  });

  it("accepts basic auth header", () => {
    const request = new Request("https://example.com/api/v1/entries", {
      method: "POST",
      headers: { Authorization: `Basic ${btoa("secret:ignored")}` }
    });
    expect(requireWriteAuth(request, env)).toBeNull();
  });

  it("rejects protected reads by default", () => {
    const request = new Request("https://example.com/api/v1/status.json");
    expect(requireReadAuth(request, env)?.status).toBe(401);
  });

  it("allows public reads when configured", () => {
    const request = new Request("https://example.com/api/v1/status.json");
    expect(requireReadAuth(request, { ...env, READ_PUBLIC: "true" } as Env)).toBeNull();
  });

  it("returns CORS headers on options", () => {
    const response = optionsResponse();
    expect(response.status).toBe(204);
    expect(response.headers.get("Access-Control-Allow-Methods")).toContain("OPTIONS");
  });
});
